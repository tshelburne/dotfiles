...
`circadian-os` `apps/ios/test/fixtures/device-api.ts` (typed `mock.fn<T>()`
factory, `DeepPartial` overrides, no cast) and `packages/testing/src/dates.ts`
(`createTestClock`); the "never typecast" rule and its runtime-hang origin
(`48ebd571`) are codified in its `.claude/CLAUDE.md`. `notched.fit` and
`stacked.bar` both use `node:test` with local `Partial`-override factories;
`stacked.bar` has repo-root Playwright e2e with a fixture that fails on console
errors and auth reused via `storageState`.
